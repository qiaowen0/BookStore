...
public class Searchbooks extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		String sql = "SELECT count(*) FROM booktable where Bookname like \""+"%"+ searchitem + "%"+"\";";
		rs = stmt.executeQuery(sql);
		rs.next();
		String ColSum = rs.getString(1);
		int colsum = Integer.parseInt(ColSum);
		sql = "SELECT * FROM booktable where Bookname like \"" +"%"+ searchitem
				+ "%"+"\";";
		rs = stmt.executeQuery(sql);
		rs.next();
		// 这里是否要先next？
